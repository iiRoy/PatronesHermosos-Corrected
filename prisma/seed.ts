...
async function main() {
  console.log('🧨 Borrando registros previos...');
    ...
    const coordinators = JSON.parse(fs.readFileSync('./prisma/seed/venue_coordinators.json', 'utf-8'));
    for (const coordinator of coordinators) {
      const expectedVenueId = coordinator.id_venue;
      if (!venueIds.includes(expectedVenueId)) {
        console.error(`⚠️ Venue con id_venue ${expectedVenueId} no existe. Saltando coordinator: ${coordinator.name}`);
        continue;
      }
    ...
    const assistants = JSON.parse(fs.readFileSync('./prisma/seed/assistant_coordinators.json', 'utf-8'));
    for (const a of assistants) {
      const expectedVenueId = a.id_venue;
      if (!venueIds.includes(expectedVenueId)) {
        console.error(`⚠️ Venue con id_venue ${expectedVenueId} no existe. Saltando assistant coordinator: ${a.name}`);
        continue;
      }
    ...
    for (const m of mentors) {
      ...
      if (!venueIds.includes(expectedVenueId)) {
        console.error(`⚠️ Venue con id_venue ${expectedVenueId} no existe. Saltando mentor: ${m.name}`);
        continue;
      }
    ...
    for (const g of groups) {
      ...
      if (!venueIds.includes(expectedVenueId)) {
        console.error(`⚠️ Venue con id_venue ${expectedVenueId} no existe. Saltando grupo: ${g.name}`);
        continue;
      }
    ...
    for (const p of participants) {
      ...
      if (expectedGroupId === undefined || expectedGroupId === null || typeof expectedGroupId !== 'number') {
        console.error(`⚠️ id_group inválido (${expectedGroupId}) para participant: ${p.name}. Saltando...`);
        continue;
      }
      const groupExists = await prisma.groups.findUnique({ where: { id_group: expectedGroupId } });
      if (!groupExists) {
        console.error(`⚠️ Group con id_group ${expectedGroupId} no existe. Saltando participant: ${p.name}`);
        continue;
      }
    ...
    for (const c of collaborators) {
      ...
      // Check if id_group is defined and a valid number
      if (expectedGroupId === undefined || expectedGroupId === null || typeof expectedGroupId !== 'number') {
        console.error(`⚠️ id_group inválido (${expectedGroupId}) para collaborator: ${c.name}. Saltando...`);
        continue;
      }
      const groupExists = await prisma.groups.findUnique({ where: { id_group: expectedGroupId } });
      if (!groupExists) {
        console.error(`⚠️ Group con id_group ${expectedGroupId} no existe. Saltando collaborator: ${c.name}`);
        continue;
      }
