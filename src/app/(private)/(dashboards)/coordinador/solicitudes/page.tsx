...
const SolicitudesRegistroAdmin = () => {
  ...
  const [isSendEmailPopupOpen, setIsSendEmailPopupOpen] = useState(false);
  const [selectedTemplate, setSelectedTemplate] = useState<string>('sol_cambio');
  const [emailFormData, setEmailFormData] = useState<any>({});
  const [emailVenueGroups, setEmailVenueGroups] = useState<GroupOption[]>([]);
  const [emailSelectedVenue, setEmailSelectedVenue] = useState<string>('');
  const [emailSelectedGroupId, setEmailSelectedGroupId] = useState<number | null>(null);
  const [emailGroupInfo, setEmailGroupInfo] = useState<any>(null);
  ...
  const [allSedesData, setAllSedesData] = useState<Sede[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        let pendingParticipants = participantsData.dataForRequests.filter(
        ...
        if (userRole === 'venue_coordinator' && coordinatorVenueId) {
          pendingParticipants = pendingParticipants.filter(
            (p: Participante) => p.groups?.venues?.id_venue === coordinatorVenueId,
          );
        }
        ...
        let pendingCollaborators = collaboratorsData.dataForRequests.filter(
          (c: ApoyoStaff) => c.status === 'Pendiente',
        );

        // Filtrar colaboradores por sede de la coordinadora
        if (userRole === 'venue_coordinator' && coordinatorVenueId) {
          pendingCollaborators = pendingCollaborators.filter(
            (c: ApoyoStaff) => c.groups?.venues?.id_venue === coordinatorVenueId,
          );
        }
