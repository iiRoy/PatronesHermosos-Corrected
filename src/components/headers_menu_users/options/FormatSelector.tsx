...
const FormatSelector: React.FC<FormatSelectorProps> = ({ format, setFormat }) => {
  return (
    <div className='mb-4' onMouseDown={(e) => e.stopPropagation()}>
      <label className='block mb-1 text-sm font-semibold text-gray-700'>Formato de descarga:</label>
      <Select value={format} onValueChange={(v) => setFormat(v as 'png' | 'jpg' | 'pdf')}>
        ...
        <SelectContent className='bg-[#f4edf4] text-[#822f87] border border-purple-300 rounded-md shadow-md animate-fadeIn transition-all duration-200 ease-in-out'>
          <SelectItem
            value='png'
