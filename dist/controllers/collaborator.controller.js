...
const createCollaborator = async (req, res) => {
    const { name, paternal_name, maternal_name, email, phone_number, college, degree, semester, preferred_role, preferred_language, preferred_level, preferred_group, gender, } = req.body;
    try {
...
const updateCollaborator = async (req, res) => {
    const { id } = req.params;
    const { name, paternal_name, maternal_name, email, phone_number, college, degree, semester, preferred_role, preferred_language, preferred_level, preferred_group, gender, role, status, level, language, } = req.body;
    try {
...
const updateCollaboratorBasicInfo = async (req, res) => {
    const { id } = req.params;
    const { name, paternal_name, maternal_name, email, phone_number, college, degree, semester, gender, preferred_role, preferred_language, preferred_level, } = req.body;
    try {
        const updateData = {};
        if (name !== undefined)
            updateData.name = name?.trim() || null;
        if (paternal_name !== undefined)
            updateData.paternal_name = paternal_name?.trim() || null;
        if (maternal_name !== undefined)
            updateData.maternal_name = maternal_name?.trim() || null;
        if (email !== undefined)
            updateData.email = email?.trim() || null;
        if (phone_number !== undefined)
            updateData.phone_number = phone_number?.trim() || null;
        if (college !== undefined)
            updateData.college = college?.trim() || null;
        if (degree !== undefined)
            updateData.degree = degree?.trim() || null;
        if (semester !== undefined)
            updateData.semester = semester?.trim() || null;
        if (gender !== undefined)
            updateData.gender = gender?.trim() || null;
        if (preferred_role !== undefined)
            updateData.preferred_role = preferred_role?.trim() || null;
        if (preferred_language !== undefined)
            updateData.preferred_language = preferred_language?.trim() || null;
        if (preferred_level !== undefined)
            updateData.preferred_level = preferred_level?.trim() || null;
...
const approveCollaborator = async (req, res) => {
    ...
    try {
        ...
        const selectedLevel = collaborator.preferred_level !== 'Pendiente' &&
            ...
            : group.level && validLevels.includes(group.level)
                ? group.level
                : 'Básico';
        const selectedLanguage = collaborator.preferred_language !== 'Pendiente' &&
