    ...
    try {
        ...
        switch (page) {
            case 'evento': {
                ...
                const [raw] = await prisma.$queryRawUnsafe(`CALL resumen_evento(?, ?, ?, ?)`, role, email, idSede, rolColab);
            ...
            case 'colaboradoras': {
                ...
                const [raw] = await prisma.$queryRawUnsafe(`CALL resumen_colaboradoras(?, ?, ?)`, role, email, sedeId);
                const resumenRaw = raw.f0;
                const resumenParsed = JSON.parse(resumenRaw);
            ...
            case 'coordinadoras': {
                const sedeId = req.query.id ? Number(req.query.id) : null;
                const [raw] = await prisma.$queryRawUnsafe(`CALL resumen_coordinadoras(?, ?, ?)`, role, email, sedeId);
                const resumenRaw = raw.f0;
                const resumenParsed = JSON.parse(resumenRaw);
                return res.json({ resumenColaboradoras: resumenParsed });
            }
            case 'times': {
                ...
                const [raw] = await prisma.$queryRawUnsafe(`CALL resumen_tiempo_roles_detallado(?, ?, ?, ?, ?, ?)`, role, email, frecuencyStr, levelStr, sedeId, stateStr);
                const resumenRaw = raw.f0;
                const resumenParsed = JSON.parse(resumenRaw);
