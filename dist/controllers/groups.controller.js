...
const createGroup = async (req, res) => {
    try {
        ...
        const { name, max_places, occupied_places, location, start_date, end_date, start_hour, end_hour, id_mentor, language, level, mode, id_venue, } = req.body;
        // Validar que todos los campos estén presentes
        const missingFields = [];
        if (!name)
            missingFields.push('name');
        if (max_places === undefined || max_places === null)
            missingFields.push('max_places');
        ...
        if (!start_hour)
            missingFields.push('start_hour');
        if (!end_hour)
            missingFields.push('end_hour');
        if (id_mentor === undefined || id_mentor === null)
            missingFields.push('id_mentor');
        if (!language)
...
const updateGroup = async (req, res) => {
    try {
        ...
        if (max_places !== undefined)
            updateData.max_places = parseInt(max_places);
        if (occupied_places !== undefined)
            updateData.occupied_places = parseInt(occupied_places);
        if (id_mentor !== undefined)
            updateData.id_mentor = parseInt(id_mentor);
        if (id_venue !== undefined)
            updateData.id_venue = parseInt(id_venue);
